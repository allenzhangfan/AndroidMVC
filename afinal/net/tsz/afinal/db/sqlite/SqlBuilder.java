...
public class SqlBuilder {
	...
	public static List<KeyValue> getSaveKeyValueListByEntity(Object entity) {
		...
		if (!(idvalue instanceof Integer && id.isAutoincrement())) { // 用了非自增长,添加id , 采用自增长就不需要添加id了
	...
	/**
	 * 返回sql：select max(id) from table
	 * <p>
	 * 
	 * @author zhangfan 2013-12-17
	 * @param clazz
	 * @return
	 */
	public static SqlInfo getSelectMaxIdSqlInfo(Class<?> clazz) {
		TableInfo table = TableInfo.get(clazz);
		StringBuffer strSQL = new StringBuffer("SELECT max(").append(table
				.getId().getColumn());
		strSQL.append(") FROM ").append(table.getTableName());
		SqlInfo sqlInfo = new SqlInfo();
		sqlInfo.setSql(strSQL.toString());
		return sqlInfo;
	}

	public static SqlInfo getMaxIdColumnSqlInfo(Class<?> clazz) {
		TableInfo table = TableInfo.get(clazz);
		StringBuffer strSQL = new StringBuffer("max(").append(table.getId()
				.getColumn());
		strSQL.append(")");
		SqlInfo sqlInfo = new SqlInfo();
		sqlInfo.setSql(strSQL.toString());
	...
	public static SqlInfo getSaveOrUpdateSqlAsSqlInfo(Object entity) {
		TableInfo table = TableInfo.get(entity.getClass());
		Object idvalue = table.getId().getValue(entity);
	...
	public static String getCreatTableSQL(Class<?> clazz) {
		...
		for (Property property : propertys) {
			...
			if (dataType == int.class || dataType == Integer.class
				...
				strSQL.append(" DOUBLE");
			} else if (dataType == boolean.class || dataType == Boolean.class) {
				strSQL.append(" BOOL");
			} else if ((Short.TYPE == dataType) || (Short.class == dataType)) {
				strSQL.append(" INTEGER");
			} else if (Date.class == dataType) {
				strSQL.append(" DATETIME");
			} else if (Blob.class == dataType) {
				strSQL.append(" BLOB");
